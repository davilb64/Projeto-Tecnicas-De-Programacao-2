...
public class UsuarioController {
   ...
   * </ul>
   *
   * @param dto objeto envelopado contendo nome, email e
   *            senha enviados no corpo da requisição (POST).
   ...
   * <ol>
   *   <li>O {@code UsernamePasswordAuthenticationToken}
   *   encapsula as credenciais brutas do DTO.</li>
   *   <li>O {@code authenticationManager.authenticate()} delega a verificação ao Spring Security,
   *       que busca o usuário pelo e-mail (UserDetailsService)
   *       e compara o hash da senha (PasswordEncoder).</li>
   *   <li>Se as credenciais forem inválidas,
   *   uma exceção de segurança é lançada e o fluxo é interrompido.</li>
   *   <li>Se forem válidas, o objeto de Autenticação
   *   é retornado contendo a entidade Usuario (Principal).</li>
   *   <li>O {@code tokenService} gera e assina o JWT com base
   *   na entidade autenticada, retornando a string ao cliente.</li>
   * </ol>
   */
  @PostMapping("/login")
  ...
  /**
   * Endpoint para consultar usuários pendentes de promoção (GET).
   */
  ...
  /**
   * Endpoint para promover usuários pendentes de promoção (PATCH).
   */
  ...
  /**
   * Endpoint para rejeitar promoção de usuários pendentes de promoção (PATCH).
   */
