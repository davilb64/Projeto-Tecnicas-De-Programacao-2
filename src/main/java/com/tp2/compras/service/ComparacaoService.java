...
public class ComparacaoService {
  ...
  /**
   * Compara as listas para encontrar mercado mais barato.
   **/
  public List<ComparacaoMercadoDTO> compararListaNosMercados(Long listaId) {
    ...
    // Mapa rápido para sabermos a quantidade que o usuário quer de cada item
    ...
    for (Estabelecimento mercado : mercados) {
      // Usa a query customizada que traz apenas o preço mais recente
      List<Preco> precosAtuais = precoRepository
              .buscarPrecosAtuaisDaListaNoMercado(variacaoIds, mercado.getId());
    ...
    comparacoes.sort((c1, c2) -> {
      ...
      if (c1.itensEncontrados() == 0 && c2.itensEncontrados() > 0) {
        return 1;
      }
      if (c2.itensEncontrados() == 0 && c1.itensEncontrados() > 0) {
        return -1;
      }
