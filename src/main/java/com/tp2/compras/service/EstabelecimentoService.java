...
public class EstabelecimentoService {
  ...
  @Transactional
    ...
    if (estabelecimentoRepository.existsByNomeIgnoreCase(dto.nome())) {
      throw new IllegalArgumentException("Já existe um estabelecimento "
               + "cadastrado ou sugerido com este nome.");
    }
   ...
   */
  @Transactional
    public void solicitar(EstabelecimentoCadastroDTO dto) {
    Assert.notNull(dto, "O DTO de solicitação não pode ser nulo");

    if (estabelecimentoRepository.existsByNomeIgnoreCase(dto.nome())) {
      throw new IllegalArgumentException("Este mercado já existe no "
                + "sistema ou está na fila de aprovação.");
    }
    ...
    EstabelecimentoResponseDTO.daEntidade(estabelecimentoRepository.save(novo));
  }
  ...
  /**
   * Lista estabelecimentos do CATÁLOGO OFICIAL (aprovado = true) por id.
   */
  ...
  /**
   * Lista estabelecimentos do CATÁLOGO OFICIAL (aprovado = true),
   * permitindo busca parcial (EU007).
   *
  ...
  public List<EstabelecimentoResponseDTO> listar(String nomeBusca) {
    ...
    if (nomeBusca != null && !nomeBusca.isBlank()) {
      resultados = estabelecimentoRepository
              .findByNomeContainingIgnoreCaseAndAprovadoTrue(nomeBusca);
    } else {
  ...
  @Transactional
    ...
    if (dto.nome() != null
      ...
      if (!estabelecimento.getNome().equalsIgnoreCase(dto.nome())
        ...
        throw new IllegalArgumentException("Já existe outro "
                        + "estabelecimento com este nome.");
      }
