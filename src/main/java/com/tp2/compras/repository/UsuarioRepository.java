...
public interface UsuarioRepository extends JpaRepository<Usuario, Long> {
   ...
   *       passando o parâmetro de forma
   *       segura via prepared statement — sem risco de SQL injection.</li>
   *   <li>A coluna email possui restrição
   *   UNIQUE no banco (definida em V1__create_usuarios_table.sql),
   *       portanto a query retorna no máximo um registro.</li>
   *   <li>Spring Data JPA envolve o resultado em
   *   {@code Optional}: se nenhum registro for encontrado,
   ...
   * <p><b>Argumentação da corretude:</b>
   * <ol>
   *   <li>Spring Data JPA traduz este método em
   *   {@code SELECT COUNT(*) > 0 FROM usuarios WHERE email = ?},
   ...
   *       o que é a semântica esperada para a validação
   *       de duplicidade no fluxo de cadastro (EU001).</li>
